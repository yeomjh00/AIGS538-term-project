...
class CutMix:
    ...
    def __call__(self, x, y, attack=False):
        ...
        for i in range(aug_size):
            ...
            src_img = images[perm[i+1]].clone()
            temp_img = src_img.cpu().numpy().transpose(1, 2, 0)
            ...
            x1 = np.clip(x - actual_w_edge // 2, 0, self.w)
            x2 = np.clip(x + actual_w_edge // 2, 0, self.w)
            y1 = np.clip(y - actual_h_edge // 2, 0, self.h)
            y2 = np.clip(y + actual_h_edge // 2, 0, self.h)

            crop =  next_img[:, x1:x2, y1:y2].clone().detach()
            img[:, x1:x2, y1:y2] = crop[:,:,:]
